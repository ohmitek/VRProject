    ...
    public class ConvaiActionsHandler : MonoBehaviour
    {
        ...
        private IEnumerator DoAction(ConvaiAction action)
        {
            ...
            switch (action.Verb)
            {
                ...
                case ActionChoice.Show:
                    // Call the Show function and yield until it's completed
                    yield return Show(action.Target);
                    break;
            }
        ...
        private IEnumerator Show(GameObject target)
        {
            ActionStarted?.Invoke("Show", target);

            // If the target is null or not active, exit the coroutine.
            if (target == null || !target.activeInHierarchy)
            {
                Logger.DebugLog("Show target is null or inactive.", Logger.LogCategory.Actions);
                yield break;
            }

            // Log that we are starting the Show action.
            Logger.DebugLog($"Showing Target: {target.name}", Logger.LogCategory.Actions);
            ...
            // Start the "Point" animation with a cross-fade transition.
            Animator animator = _currentNPC.GetComponent<Animator>();
            animator.CrossFade(Animator.StringToHash("Point"), 0.1f);

            // Wait for one frame to ensure that the Animator has had time to transition
            // to the "Point" animation state.
            ...
            if (clipInfo == null || clipInfo.Length == 0)
            {
                Logger.DebugLog("No animation clips found for Point state!", Logger.LogCategory.Actions);
                yield break;
            }

            // Store the length of the "Point" animation clip for later use.
            float pointClipLength = clipInfo[0].clip.length;

            // Wait for the duration of the "Point" animation.
            yield return new WaitForSeconds(pointClipLength);

            // Transition back to the "Idle" animation.
            animator.CrossFade(Animator.StringToHash("Idle"), 0.1f);

            ActionEnded?.Invoke("Show", target);
        }
